...
if [ $machine = "WCOSS_C" ] ; then
  ...
  . $MODULESHOME/init/sh 2>/dev/null
  module use /glade/p/ral/jnt/tools/nwprod.update2018/lib/modulefiles 2>/dev/null
# Not on cheyenne  module load esmf/7.1.0bs34 2>/dev/null
fi
...
NLN=${NLN:-"/bin/ln -sf"}
NMV=${NMV:-"/bin/mv"}
SEND=${SEND:-"NO"}   #move final result to rotating directory
...
APRUN_FV3=${FCST_LAUNCHER:-${APRUN_FCST:-${APRUN:-""}}}
NTHREADS_FV3=${nth_f:-${nth_f:-1}}
cores_per_node=${npe_node_f:-${task_per_node:-24}}
ntiles=${ntiles:-6}
NTASKS_FV3=${tasks:-$((6*layout_x*layout_y))}
...
if [ $warm_start = ".false." ]; then
  if [ -d $ICSDIR/$CDATE/$CDUMP/$CASE/INPUT ]; then
    echo "Copying files from $ICSDIR/$CDATE/$CDUMP/$CASE/INPUT to $DATA/INPUT"
...
$NLN $FIX_AM/ozprdlos_2015_new_sbuvO3_tclm15_nuchem.f77 $DATA/INPUT/global_o3prdlos.f77
$NLN $FIX_AM/global_h2o_pltc.f77                        $DATA/INPUT/global_h2oprdlos.f77
...
if [ $(echo $MONO | cut -c-4) = "mono" ];  then # monotonic options
  ...
  do_vort_damp=".true."
  if [ ${TYPE} = "nh" ]; then # non-hydrostatic
    hord_mt=${hord_mt_nh_nonmono:-"6"}
    hord_xx=${hord_xx_nh_nonmono:-"6"}
  else # hydrostatic
